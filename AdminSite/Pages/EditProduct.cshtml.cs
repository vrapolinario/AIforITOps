...
public class EditProductModel : PageModel
{
    ...
    {
        _logger.LogInformation("OnPostGenerateDescriptionAsync called in EditProductModel");
        try
        {
            string body;
    ...
    var connStr = System.IO.File.ReadAllText("/mnt/secrets-store/CosmosDBConnectionString");
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            {
                var routeIdObj = RouteData.Values["id"];
                if (routeIdObj is string idStr && !string.IsNullOrEmpty(idStr))
                    Product.id = idStr;
                else
        ...
        string connStr = System.IO.File.ReadAllText("/mnt/secrets-store/CosmosDBConnectionString");
        string dbName = Environment.GetEnvironmentVariable("COSMOSDB_DATABASE_NAME");
        string containerName = Environment.GetEnvironmentVariable("PRODUCTS_CONTAINER_NAME");
        var client = new Microsoft.Azure.Cosmos.CosmosClient(connStr);
        var container = client.GetContainer(dbName, containerName);
        ...
        else
        {
            ...
            var existing = await container.ReadItemAsync<AdminSite.Models.Product>(Product.id, new Microsoft.Azure.Cosmos.PartitionKey(Product.id));
            Product.ImageData = existing.Resource.ImageData;
        }
