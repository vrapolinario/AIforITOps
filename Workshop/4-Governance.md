...
- Configure Microsoft Foundry for Data Loss Prevention (DLP).
- Check Policy status to confirm resources are compliant.

## Configure Azure policy for Microsoft Foundry

With Azure Policy, you can check and/or enforce resource compliance at subscription or resource group level. Azure provides built-in policies for Azure services that can be leveraged.

- On the Azure portal, navigate to the Microsoft Foundry service. (Note: Look for the Foundry resource of type "Foundry" and not "Foundry Project".)
- On the Microsoft Foundry service page, click the JSON View on the right-hand side under Essentials.
- On the right-hand side panel for Resource JSON, take a note of the type: Microsoft.CognitiveServices/accounts.
- Close the Microsoft Foundry page.
- On the Azure portal, type "Policy" on the Search bar at the top-center and select Policy.
- On the Azure Policy page, select Compliance on the left-hand side menu and click Assign policy.
...
- On Policy enforcement switch to Do not enforce. Note: This will effectively set this policy as Assessment only. Click Next.
...
## Set up Data loss prevention for Microsoft Foundry

Data loss prevention should encompass a series of analysis and tactics to prevent data from being leaked, stolen or deleted from your environment. From an AI service standpoint, Microsoft Foundry can add an additional layer of protection by preventing your data from leaving the environment.
...
- The above command checks for the Public Network Access policy configured for the Microsoft Foundry service. By default, this is set as Disabled, which means data can flow from the Microsoft Foundry service to any address on the internet.
- On the PowerShell session, type:
...
Note: To revert back to the default settings, you can run the following command:

```powershell
az rest --method patch --uri "/subscriptions/{your-subscription-id}/resourceGroups/{resource-group-name}/providers/Microsoft.CognitiveServices/accounts/{openai-service-name}?api-version=2024-10-01" --headers "Content-Type=application/json" --body '{\"properties\": { \"publicNetworkAccess\": \"Disabled\", \"restrictOutboundNetworkAccess\": false }}'
```
